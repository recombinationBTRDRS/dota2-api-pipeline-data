...
class AsyncOpenDotaClient:
    ...
    async def get_match(self, match_id: int) -> dict:
        ...
        Виконує retry при 429 та 5xx з exponential backoff.
        Читає base URL і timeout з settings.
        ...
        Raises:
            RuntimeError: якщо всі спроби вичерпані.
        """
        ...
        async with httpx.AsyncClient(timeout=settings.OPENDOTA_TIMEOUT) as client:
            for attempt in range(1, settings.OPENDOTA_RETRIES + 1):
                try:
                    ...
                    sleep = (2 ** attempt) + random.random()
                    logger.warning(
                        "AsyncOpenDota error %s on attempt %s/%s, backoff %.2fs",
                        type(e).__name__,
                        attempt,
                        settings.OPENDOTA_RETRIES,
                        sleep,
